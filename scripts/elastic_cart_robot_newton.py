...
MOTOR_STIFFNESS = 30000.0
MOTOR_DAMPING = 500.0
MOTOR_FORCE_LIMIT = 2000.0
ELASTIC_FORCE_LIMIT = 2000.0

MASS_LINK_X_MOTOR = 2.0
MASS_LINK_X = 2.0
MASS_LINK_Y_MOTOR = 1.5
MASS_LINK_Y = 2.0
...
# Encoder and noise constants (moved to module level for accessibility)
...
def main():
    ...
    for _ in range(num_steps):
        ...
        if REF_MODE == 0:
            ref_x, ref_y, ref_z = initial_motor_reference
            vel_x = 0.0
            vel_y = 0.0
            vel_z = 0.0
        elif REF_MODE == 1:
            ramp = 1 - np.exp(-time / 1.0)

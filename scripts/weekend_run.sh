...
PLATFORMS=(kuka_lbr_iiwa_14_r820_table ur10_table_round5)
...
check_env() {
  ...
  "${SIM_PY[@]}" -c 'import pinocchio, mujoco, numpy, pandas; print("sim deps ok")' || return 1
  "${NN_PY[@]}" -c 'import torch, optuna; print("torch", torch.__version__, "cuda", torch.cuda.is_available())' || return 1
  "${SIM_PY[@]}" scripts/diagnose_controller_modes.py --help >/dev/null || return 1
  "${NN_PY[@]}" "$NN_DIR/optuna_search_lite.py" --help >/dev/null || return 1
}
